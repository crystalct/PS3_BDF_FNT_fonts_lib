...
int main(int argc,const char *argv[])
{
	...
	*wait_label = sLabelValue;
	...
	fnt_class alpha;
	init_fnt(gGcmContext, display_width, display_height, NULL, 32, 2, &alpha);
	...
	int ret = addfnt_from_file_fnt(&alpha, "/dev_hdd0/game/FBNE00123/USRDIR/12x24a.fnt", 2);
	if (!ret) {
	...
	ret = addfnt_from_file_fnt(&alpha, "/dev_hdd0/game/FBNE00123/USRDIR/b24.fnt", 0);
	if (!ret) {
	...
	addfnt_from_bitmap_array_fnt(&alpha, font, 32, 255, 16, 32, 2, BIT0_FIRST_PIXEL, 2);
	addfnt_from_bitmap_array_fnt(&alpha, (u8*)data_font_Adonais, 0x20, 0x7e, 32, 31, 1, BIT7_FIRST_PIXEL, 2);
